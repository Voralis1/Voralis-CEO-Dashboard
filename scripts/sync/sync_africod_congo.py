...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        "Avec --since : rattrapage historique ponctuel. Avec --full-rescan : rescan périodique de "
        "tout l'historique (voir avertissement ci-dessous)."
    )
...
def main() -> None:
    ...
    def relogin() -> str:
        new_token, _ = af_login(BASE_URL, email, password, user_agent=USER_AGENT, origin=ORIGIN)
        return new_token
    ...
    try:
        ...
        if args.full_rescan:
            supabase_prune_missing("africod_congo_leads", "order_id", ids)
