...
create table if not exists coliscod_leads (
  ...
  -- Délai 1er contact (2026-07-14) : voir kpi_coliscod_marche_periode pour le calcul et son biais
  -- documenté (fiable seulement pour no_answer_count <= 1 — même limite que ClickMarket).
  no_answer_count integer,
  last_unreached_date timestamptz,
  synced_at timestamptz not null default now()
);

alter table coliscod_leads add column if not exists no_answer_count integer;
alter table coliscod_leads add column if not exists last_unreached_date timestamptz;
...
--   - Décalage horaire WAT (2026-07-14) : l'Angola est en WAT (UTC+1). Nos colonnes timestamptz
--                       sont stockées en UTC ; caster directement `col::date` lit donc
--                       "aujourd'hui" avec 1h de retard par rapport à l'heure locale. Vérifié
--                       précisément sur "Aujourd'hui" : filtre UTC = 68, filtre décalé +1h = 72,
--                       widget natif Coliscod = 72 (match exact). Fix appliqué aux 3 dates
--                       concernées (order_created_at, confirmed_at, delivered_at) en castant
--                       `(col + interval '1 hour')::date` au lieu de `col::date`.
