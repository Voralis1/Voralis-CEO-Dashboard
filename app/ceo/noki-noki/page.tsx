...
import { fetchAfricodCongoKpis, AfricodCongoKpiRow } from "@/lib/supabase/queries";
...
export default function AfricodCongoPage() {
  const { dateFrom, dateTo } = useFilters();
  const [rows, setRows] = useState<AfricodCongoKpiRow[]>([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const data = await fetchAfricodCongoKpis(dateFrom, dateTo);
  ...
  return (
    <div>
      <Topbar title="Africod Congo" subtitle="AfriqueCOD · Commandes confirmées et revenus par pays" />
      ...
      <div className="px-6 py-5 space-y-5">
        ...
        {loading && rows.length === 0 && !error && (
          <div className="flex items-center justify-center py-16 text-slate-400 gap-2">
            ...
            <span className="text-sm">Chargement des données Africod Congo…</span>
          </div>
        )}

        {!loading && !error && rows.length === 0 && (
          <p className="text-sm text-slate-500">Aucune commande Africod Congo pour cette période.</p>
        )}

        {rows.length > 0 && (
          <>
            <Section title="Performance Africod Congo par pays">
