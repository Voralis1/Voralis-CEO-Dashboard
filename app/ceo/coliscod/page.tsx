...
import { fetchColiscodKpis, ColiscodKpiRow } from "@/lib/supabase/queries";
import { Loader2, AlertTriangle } from "lucide-react";

function fmtAOA(v: number): string {
  return Math.round(v).toLocaleString("fr-FR").replace(/\s/g, " ") + " AOA";
}
...
export default function ColisCodPage() {
  const { dateFrom, dateTo } = useFilters();
  const [rows, setRows] = useState<ColiscodKpiRow[]>([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const data = await fetchColiscodKpis(dateFrom, dateTo);
  ...
  return (
    <div>
      <Topbar title="Coliscod Angola" subtitle="AfricaCOD · Commandes confirmées et revenus par pays" />
      ...
      <div className="px-6 py-5 space-y-5">
        ...
        {loading && rows.length === 0 && !error && (
          <div className="flex items-center justify-center py-16 text-slate-400 gap-2">
            ...
            <span className="text-sm">Chargement des données Coliscod…</span>
          </div>
        )}

        {!loading && !error && rows.length === 0 && (
          <p className="text-sm text-slate-500">Aucune commande Coliscod pour cette période.</p>
        )}

        {rows.length > 0 && (
          <>
            <Section title="Performance Coliscod Angola par pays">
                  ...
                  <tbody>
                    {sorted.map((r) => (
                      <tr key={r.country_id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-3 py-3 text-slate-700">{fmtAOA(r.ca_livre)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
